class Calc
        {
            ...
            private void Check()
            {
                ...
                if (_entry.IndexOf(',')!=_entry.LastIndexOf(',')) // Проверка на то, что есть только одна запятая или её нет вообще
                    throw new Exception("Возможна только одна запятая в числе!");
                ...
                foreach (char c in _entry) // Проверка на верную СС
                    if ((alphabet.IndexOf(c) > _inputNumSyst - 1 || alphabet.IndexOf(c) == -1) & c != '.')
                        throw new Exception("Одна из цифр не соответствует введённой системе счисления (" + c + ")!");
            }

            public string Convert()
            {
                return Data.Convert(new Data(_entry, _inputNumSyst), _outputNumSyst);
            }
        }